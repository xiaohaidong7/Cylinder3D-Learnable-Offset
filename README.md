...
## Learnable Cylindrical Offset Extension

This repository is an Apache-2.0 derivative of the original
[Cylinder3D](https://github.com/xinge008/Cylinder3D) implementation. It adds an
optional learnable local deformation of cylindrical point coordinates for
SemanticPOSS experiments while retaining the original sparse voxel assignment.

For every point, a small MLP predicts bounded offsets in `(rho, theta, z)`. The
offsets are limited to half of one cylindrical voxel with `tanh`, applied to the
local and absolute cylindrical features, and used to reconstruct corrected
Cartesian `(x, y)` features. The normalized offsets are also appended as a
three-dimensional spatial embedding before point-to-voxel feature aggregation.
The offset head is zero-initialized so training starts from fixed cylindrical
coordinates.

Relevant files:

- `network/cylinder_fea_generator.py`: offset prediction and feature deformation
- `builder/model_builder.py`: optional module construction
- `config/config.py`: strict YAML schema entries
- `config/semanticposs_learnable_offset_b1_full.yaml`: full 40-epoch experiment
- `config/semanticposs_repro_b1_full.yaml`: fixed-coordinate baseline

Prepare SemanticPOSS under `data/SemanticPOSS/sequences/`, then run:

```bash
bash train_semanticposs.sh 0 config/semanticposs_learnable_offset_b1_full.yaml
```

Run a one-batch forward/backward verification with:

```bash
python smoke_learnable_offset.py
```

Datasets, checkpoints, and training logs are intentionally excluded from this
repository. The original Cylinder3D authors and citation are retained below.
